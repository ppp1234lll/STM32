...
{
    DEM_CR |= (uint32_t)DEM_CR_TRCENA;		/* 使能DWT外设, 另外内核调试寄存器DEMCR的位24控制，写1使能。*/
    DWT_CYCCNT = (uint32_t)0u;				/* DWT CYCCNT寄存器计数清0 */
    DWT_CR |= (uint32_t)DWT_CR_CYCCNTENA;   /* 使能Cortex-M DWT CYCCNT寄存器, 由DWT的CYCCNTENA 控制，也就是DWT控制寄存器的位0控制，写1使能。 */
}       
