...
//	res = OSTaskCreate(tcp_client_thread,(void*)0,(OS_STK*)&TCPCLIENT_TASK_STK[TCPCLIENT_STK_SIZE-1],TCPCLIENT_PRIO); //创建TCP客户端线程
	res = OSTaskCreateExt(tcp_client_thread, 															//建立扩展任务(任务代码指针) 
										(void *)0,																					//传递参数指针 
										(OS_STK*)&TCPCLIENT_TASK_STK[TCPCLIENT_STK_SIZE-1], 					//分配任务堆栈栈顶指针 
										(INT8U)TCPCLIENT_PRIO, 															//分配任务优先级 
										(INT16U)TCPCLIENT_PRIO,															//(未来的)优先级标识(与优先级相同) 
										(OS_STK *)&TCPCLIENT_TASK_STK[0], 											//分配任务堆栈栈底指针 
										(INT32U)TCPCLIENT_STK_SIZE, 															//指定堆栈的容量(检验用) 
										(void *)0,																					//指向用户附加的数据域的指针 
										(INT16U)OS_TASK_OPT_STK_CHK|OS_TASK_OPT_STK_CLR);		//建立任务设定选项 	
	
	OSTaskNameSet(TCPCLIENT_PRIO, (INT8U *)(void *)"tcp_client", &err);
