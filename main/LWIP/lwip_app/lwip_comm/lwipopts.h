...
#define MEMP_NUM_UDP_PCB        15		// MEMP_NUM_UDP_PCB:UDP协议控制块(PCB)数量.每个活动的UDP"连接"需要一个PCB.
#define MEMP_NUM_TCP_PCB        15		// MEMP_NUM_TCP_PCB:同时建立激活的TCP数量
#define MEMP_NUM_TCP_PCB_LISTEN 9			// MEMP_NUM_TCP_PCB_LISTEN:能够监听的TCP连接数量
#define MEMP_NUM_TCP_SEG        23		// MEMP_NUM_TCP_SEG:最多同时在队列中的TCP段数量
#define MEMP_NUM_SYS_TIMEOUT    12// MEMP_NUM_SYS_TIMEOUT:能够同时激活的timeout个数


#define MEMP_NUM_NETBUF         23     // 使用netconn和socket编程时，该值的大小会影响可以同时创建的连接的个数
#define MEMP_NUM_NETCONN        30     // 使用netconn和socket编程时，该值设置太小，可能导致接收数据时分配内存失败，从而不能同时为几个连接的数据收发服务


//pbuf选项
#define PBUF_POOL_SIZE     30    // pbuf内存池中的pbuf的最大个数
...
#define IP_SOF_BROADCAST  1 // 开启广播功能
...
////KEEPALIVE
#define LWIP_TCP_KEEPALIVE              1    			 // 启用keepalive功能
#define TCP_KEEPIDLE_DEFAULT            7200000UL  // 3秒内双方无数据则发起保活探测
#define TCP_KEEPINTVL_DEFAULT           600000UL    // 10min发送一次保活探测
#define TCP_KEEPCNT_DEFAULT             300        // 300次探测无响应则断开
