...
#if	ONVIF_MALLOC == 0
uint8_t ONVIF_RECV[TCP_RX_BUFSIZE] = {0};
#endif


void onvif_tcp_timer_task(void)
{
	sg_ipc_flag.timeout = 1;	//接收超时处理
	TIM4_DISABLE;  			 	  	//关闭定时器，停止请求数据
}

int onvif_send(int sock_t,char *onvif_cmd,uint8_t sort,char *ip,int port)
{
	char *onvif_send_data;
	int str_len    = 0;
...
int ONVIF_IPC_GetDevice_API(int socket_t,char *buff,char *ip,int port)
{
...
#if	ONVIF_MALLOC == 1	// 动态内存管理
	rcvData = mymalloc(SRAMIN,TCP_RX_BUFSIZE);  // 申请内存
#else
	rcvData = ONVIF_RECV;
#endif
	...
	if(rcvData != NULL)
	{
		sg_ipc_flag.timeout = 0;
		TIM4_ENABLE; // 开始超时检测
		while(1)
		{
			recv_len = recv(socket_t,rcvData+data_len, TCP_RX_BUFSIZE-data_len, 0);	
			if(recv_len > 0)
				data_len += recv_len;
			if(ONVIF_TCP_DEBUG) printf("recv_len1:%d \n",recv_len);	
			if(recv_len == 0) // 连接关闭
			{
				TIM4_DISABLE;
				break;
			...
			{
				if(sg_ipc_flag.timeout == 1)
					break;
			}
		}
		if(data_len > 0)
		{
			if(ONVIF_TCP_DEBUG) printf("Device data_len:%d \n",data_len);
			ret = Get_IPC_Device_Info((char*)rcvData);    // 处理数据，获得设备参数
		}
		else
		{
			ret = -7;
			if(ONVIF_TCP_DEBUG) printf("not device recv:%d\n",recv_len);
		}
		closesocket(socket_t); // 关闭连接
#if	ONVIF_MALLOC == 1
	...
	{
		if(ONVIF_TCP_DEBUG) printf("malloc error\n");
#if	ONVIF_MALLOC == 1
    myfree(SRAMIN,rcvData);   	// 释放内存
#endif
		ret = -8;		
...
int ONVIF_IPC_Network_API(int socket_t,char *buff,char *ip,int port)
{
	...
	if(rcvData != NULL)
	{
		...
		{
			if(ONVIF_TCP_DEBUG) printf("Device data_len:%d \n",data_len);
			ret = Get_IPC_Network_Info((char*)rcvData);    // 处理数据，获得设备参数
		}
		else
		{
			ret = -7;
			if(ONVIF_TCP_DEBUG) printf("not network recv:%d\n",recv_len);
		}
		closesocket(socket_t); // 关闭连接
#if	ONVIF_MALLOC == 1
    myfree(SRAMIN,rcvData);   	// 释放内存
...
{	
	...
	if(rcvData != NULL)
	{
		...
		{
			if(ONVIF_TCP_DEBUG) printf("Device data_len:%d \n",data_len);
			ret = Get_IPC_Gateway_Info((char*)rcvData);    // 处理数据
		}
		else
		{
			ret = -7;
...
int ONVIF_IPC_Token_API(int socket_t,char *buff,char *ip,int port)
{
	...
	if(rcvData != NULL)
	{
		...
		{
			if(ONVIF_TCP_DEBUG) printf("Device data_len:%d \n",data_len);
			ret = Get_IPC_Config_Token((char*)rcvData);    // 处理数据，获得设备参数
		}
		else
		{
			ret = -7;
...
int ONVIF_IPC_OSD_API(int socket_t,char *buff,char *ip,int port)
{
	...
	if(rcvData != NULL)
	{
		...
		{
			if(ONVIF_TCP_DEBUG) printf("Device data_len:%d \n",data_len);
			ret = Get_IPC_OSD_Info((char*)rcvData);    // 处理数据，获得设备参数
		}
		else
		{
			ret = -7;
...
int ONVIF_IPC_Time_API(int socket_t,char *buff,char *ip,int port)
{
	...
	if(rcvData != NULL)
	{
		...
		{
			if(ONVIF_TCP_DEBUG) printf("Device data_len:%d \n",data_len);
			ret = Get_IPC_Time_Info((char*)rcvData);    // 处理数据，获得设备参数
		}
		else
		{
			ret = -7;
...
int ONVIF_IPC_Reboot_API(int socket_t,char *buff,char *ip,int port)
{
	...
	if(rcvData != NULL)
	{
		...
		{
			if(ONVIF_TCP_DEBUG) printf("Device data_len:%d \n",data_len);
			ret = Get_IPC_Reboot_Info((char*)rcvData);    // 处理数据，获得设备参数
		}
		else
		{
			ret = -7;
...
{	
	...
	if(rcvData != NULL)
	{
		...
		{
			if(ONVIF_TCP_DEBUG) printf("Device data_len:%d \n",data_len);
			ret = Get_IPC_SetTime_Info((char*)rcvData);    // 处理数据，获得设备参数
		}
		else
		{
			ret = -7;
